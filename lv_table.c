...
void lv_table_set_cell_value_static(lv_obj_t* obj, uint16_t row, uint16_t col, const char* txt) {
    ...
    if (table->cell_data[cell] != NULL) {
        ...
        lv_mem_free(table->cell_data[cell]);
        table->cell_data[cell] = lv_mem_alloc(table->cell_data[cell], 1);
    ...
    table->static_text[cell] = (char*)txt;
    refr_size(obj, row);
...
#if LV_TABLE_USE_STATICTEXT == 1
            lv_mem_free(table->static_text[i]);
#endif
        }
    }
...
#if LV_TABLE_USE_STATICTEXT == 1
    table->static_text = lv_mem_realloc(table->static_text, table->row_cnt * table->col_cnt * sizeof(char*));
    LV_ASSERT_MALLOC(table->static_text);
    if (table->static_text == NULL) return;
#endif
...
#if LV_TABLE_USE_STATICTEXT == 1
        lv_memset_00(&table->static_text[old_cell_cnt], (new_cell_cnt - old_cell_cnt) * sizeof(table->static_text[0]));
#endif
    }
...
#if LV_TABLE_USE_STATICTEXT == 1
        lv_memcpy_small(&new_static_data[new_col_start], &table->static_text[old_col_start], sizeof(new_static_data[0]) * min_col_cnt);
#endif
    }
...
#if LV_TABLE_USE_STATICTEXT == 1
    lv_mem_free(table->static_text);
    table->static_text = new_static_data;
#endif
...
#if LV_TABLE_USE_STATICTEXT == 1
    if (table->static_text) lv_mem_free(table->static_text);
#endif
}
